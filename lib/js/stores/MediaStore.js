...
var MediaStore = Reflux.createStore({
  ...
  pullMediaForUser(options) {
    this.media[options.handle] = [];
    //Instagram
    var promise = $ig.retrieveUserFeed(options)
                    .then((data) => {
                      this.media[options.handle] = data && data.map(sanitize) || [];
  ...
  pullMediaForUsers(userArray, min_timestamp, max_timestamp) {
    var userArray = ['mboprtr', 'schoolboyaustin'];
